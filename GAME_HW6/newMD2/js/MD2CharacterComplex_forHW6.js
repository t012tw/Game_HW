...
*/
	...
	this.updateMovementModel = function ( delta ) {
		...
		vel.set (Math.random()-0.5, 1, Math.random()-0.5).setLength(25+Math.random()*10);
		/*if(controls.jump){
			update(delta,this.root);
			console.log(this.root.position.y);
		}
		else{*/
			this.root.position.y = groundLevel (this.root.position.x,this.root.position.z);
		/*}
		//this.root.position.y = snoise.noise(0.5*3/40,0.5*3/40)*500; //離散Y
		
		//console.log(this.root.position.y)
		// steering
		function update (dt,mod) {
			vel.add (force.clone().multiplyScalar(dt));
			mod.position.add (vel.clone().multiplyScalar(dt)); 
			if (mod.position.y < 0) {
				mod.position.y = 0;
			}
		}*/
		...
		EatTreasure(this.root.position.x,this.root.position.y,this.root.position.z);
